...
struct TableMaker {
  ...
  Produces<ReducedTracksBarrelPOSTPID> trackBarrelPostPID;
  ...
  //new add
  Configurable<float> fConfigMaxTPCchi2{"cfgMaxTPCchi2", 4.0, "Maximum TPC chi2"};
  Configurable<float> fConfigMaxITSchi2{"cfgMaxITSchi2", 36.0, "Maximum ITS chi2"};
  Configurable<float> fConfigMaxEta{"cfgMaxEta", 0.9, "Maximum track eta"};
  //end
  ...
  //Filter barrelSelectedTracks = ifnode(fIsRun2.node() == true, aod::track::trackType == uint8_t(aod::track::Run2Track), aod::track::trackType == uint8_t(aod::track::Track)) && o2::aod::track::pt >= fConfigBarrelTrackPtLow && nabs(o2::aod::track::eta) <= fConfigMaxEta && o2::aod::track::tpcSignal >= fConfigMinTpcSignal && o2::aod::track::tpcSignal <= fConfigMaxTpcSignal && o2::aod::track::tpcChi2NCl < fConfigMaxTPCchi2 && o2::aod::track::itsChi2NCl < fConfigMaxITSchi2;
  Filter barrelSelectedTracks = o2::aod::track::tpcChi2NCl < fConfigMaxTPCchi2 && o2::aod::track::itsChi2NCl < fConfigMaxITSchi2;
  ...
  void init(o2::framework::InitContext& context)
  {
    ...
    bool enableBarrelHistos = (context.mOptions.get<bool>("processFull") || context.mOptions.get<bool>("processFullWithCov") ||
                               context.mOptions.get<bool>("processFullWithCent") || context.mOptions.get<bool>("processBarrelOnlyWithV0Bits") ||
  ...
  void fullSkimming(TEvent const& collision, aod::BCs const& bcs, TTracks const& tracksBarrel, TMuons const& tracksMuon)
  {
    ...
    if constexpr (static_cast<bool>(TTrackFillMap)) {
      ...
      if constexpr (static_cast<bool>(TTrackFillMap & VarManager::ObjTypes::TrackV0Bits)) {
        trackBarrelPostPID.reserve(tracksBarrel.size());
      }
      ...
      for (auto& track : tracksBarrel) {
        ...
        trackBarrelPID(track.tpcSignal(),
                       ...
                       track.trdSignal());
        if constexpr (static_cast<bool>(TTrackFillMap & VarManager::ObjTypes::TrackV0Bits)) {
          trackBarrelPostPID(track.pairmark());
        }
        }
