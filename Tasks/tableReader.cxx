...
using MyBarrelTracks = soa::Join<aod::ReducedTracks, aod::ReducedTracksBarrel, aod::ReducedTracksBarrelPID, aod::ReducedTracksBarrelPOSTPID>;
...
using MyBarrelTracksSelected = soa::Join<aod::ReducedTracks, aod::ReducedTracksBarrel, aod::ReducedTracksBarrelPID, aod::ReducedTracksBarrelPOSTPID, aod::BarrelTrackCuts>;
...
constexpr static uint32_t gkTrackFillMapWithV0 = VarManager::ObjTypes::ReducedTrack | VarManager::ObjTypes::ReducedTrackBarrel | VarManager::ObjTypes::ReducedTrackBarrelPID | VarManager::ObjTypes::TrackV0Bits;
...
struct AnalysisSameEventPairing {
  ...
  Produces<aod::DileptonsFlow> dileptonFlowList;
  Produces<aod::DileptonsV0> dileptonV0List;
  ...
  void init(o2::framework::InitContext& context)
  {
    ...
    // Keep track of all the histogram class names to avoid composing strings in the event mixing pairing
    TString histNames = "";
    if (context.mOptions.get<bool>("processJpsiToEESkimmed") || context.mOptions.get<bool>("processVnJpsiToEESkimmed") || context.mOptions.get<bool>("processJpsiToEECalibrationSkimmed") || context.mOptions.get<bool>("processAllSkimmed")) {
      TString cutNames = fConfigTrackCuts.value;
      if (!cutNames.IsNull()) {
        std::unique_ptr<TObjArray> objArray(cutNames.Tokenize(","));
        for (int icut = 0; icut < objArray->GetEntries(); ++icut) {
  ...
  void runSameEventPairing(TEvent const& event, TTracks1 const& tracks1, TTracks2 const& tracks2)
  {
    ...
    for (auto& [t1, t2] : combinations(tracks1, tracks2)) {
      ...
      constexpr bool trackHasV0 = ((TTrackFillMap & VarManager::ObjTypes::TrackV0Bits) > 0);
      ...
      if constexpr ((TPairType == pairTypeEE) || (TPairType == pairTypeMuMu)) { // call this just for ee or mumu pairs
        ...
        if constexpr (trackHasV0) {
          VarManager::FillPairV0<TPairType>(t1, t2);
        }
        if constexpr (eventHasQvector) {
      ...
      //new add
      if constexpr (trackHasV0) {
        bool isunlikesign = (t1.sign() * t2.sign() < 0);
        bool isrealv0 = bool(t1.pairindex()==t2.pairindex());
        if (isunlikesign & isrealv0) {
          if (VarManager::fgValues[VarManager::kv0type] > 0){
            dileptonV0List(VarManager::fgValues[VarManager::kalphav0], VarManager::fgValues[VarManager::kqtarmv0],t1.sign() + t2.sign(), VarManager::fgValues[VarManager::kv0type], t1.pt(), t1.eta(), t1.sign(),t2.pt(), t2.eta(), t2.sign(),
            t1.tpcNSigmaEl(), t1.tpcNSigmaPi(), t1.tpcNSigmaPr(), t2.tpcNSigmaEl(), t2.tpcNSigmaPi(), t2.tpcNSigmaPr(), t1.pairindex(), t2.pairindex());
          }
        }    
      }
      //end
  ...
  void processJpsiToEECalibrationSkimmed(soa::Filtered<MyEventsVtxCovSelected>::iterator const& event, soa::Filtered<MyBarrelTracksSelected> const& tracks)
  {
    VarManager::ResetValues(0, VarManager::kNVars);
    VarManager::FillEvent<gkEventFillMap>(event, VarManager::fgValues);
    runSameEventPairing<VarManager::kJpsiToEE, gkEventFillMap, gkTrackFillMapWithV0>(event, tracks, tracks);
  }
  ...
  PROCESS_SWITCH(AnalysisSameEventPairing, processJpsiToEECalibrationSkimmed, "Run electron-electron pairing for v0, with skimmed tracks",false);
    ...
    if (classStr.Contains("Track")) {
      if (classStr.Contains("Barrel")) {
        ...
        if (classStr.Contains("PIDCalibElectron")) {
          dqhistograms::DefineHistograms(histMan, objArray->At(iclass)->GetName(), "track", "tpcpostcalibelectron");
        }
        else if (classStr.Contains("PIDCalibPion")) {
          dqhistograms::DefineHistograms(histMan, objArray->At(iclass)->GetName(), "track", "tpcpostcalibpion");
        }
        else if (classStr.Contains("PIDCalibProton")) {
          dqhistograms::DefineHistograms(histMan, objArray->At(iclass)->GetName(), "track", "tpcpostcalibproton");
        }
        // else {
        //   dqhistograms::DefineHistograms(histMan, objArray->At(iclass)->GetName(), "track", "its,tpcpid,dca,tofpid");
        // }
