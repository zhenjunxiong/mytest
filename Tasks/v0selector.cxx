...
struct v0selector {
  ...
  Configurable<bool> isfillv0data{"is fill v0data table",true,"is fill v0data table"};
  ...
  void process(aod::Collisions const&, aod::BCsWithTimestamps const&, FullTracksExt const& tracks, aod::V0s const& V0s, aod::Cascades const& Cascades)
  {
    ...
    for (auto& V0 : V0s) {
      ...
      pairmark[V0.posTrackId()] = V0.globalIndex();
      pairmark[V0.negTrackId()] = V0.globalIndex();
    ...
    for (auto& track : tracks) {
      ...
      v0bits(pidmap[track.globalIndex()],pairmark[track.globalIndex()]);
    } // end of track loop
...
struct trackPIDQA {
  ...
  Produces<aod::myv0tree> v0minitree;
  Configurable<double> dwnSmplFactor{"factor of saved v0 over all v0",0.5,"factor"};
  ...
  HistogramRegistry registry{
    "registry",
    {
      ...
      {"hTrackPidbit", "", {HistType::kTH1F, {{32, 0, 32}}}},
  ...
  // void fillskimv0table(V0 const& v0,T const& track,C const& collision, const float nSigmaTPC, const float nSigmaTOF, const o2::track::PID::ID id)
  // {
  ...
  void fillskimv0table(V0 const& v0,T const& postrack,T const& negtrack,C const& collision, const float nSigmaTPC_pos, const float nSigmaTOF_pos,const float nSigmaTPC_neg, const float nSigmaTOF_neg,const o2::track::PID::ID pos_id,const o2::track::PID::ID neg_id)
  {
    ...
    const float track1_eta = postrack.eta();
    const float track1_phi = postrack.phi();
    const float track1_mass = o2::track::pid_constants::sMasses[pos_id];
    const float track1_p = postrack.tpcInnerParam();
    const float track1_bg = track1_p / track1_mass;

    const float track2_eta = negtrack.eta();
    const float track2_phi = negtrack.phi();
    const float track2_mass = o2::track::pid_constants::sMasses[neg_id];
    const float track2_p = negtrack.tpcInnerParam();
    const float track2_bg = track2_p / track2_mass;


    const double pseudoRndm = postrack.pt() * 1000. - (long)(postrack.pt() * 1000);
  ...
  {
    v0minitree.reserve(tracks.size());
    ...
    for (auto v0 : v0s) {
      ...
      // if (posTrack.pidbit() & (uint8_t(1) << v0selector::kGamma)) {
      //   fillskimv0table(v0,posTrack,collision,posTrack.tpcNSigmaEl(),posTrack.tofNSigmaEl(),o2::track::PID::Electron);
      // }
      // if (negTrack.pidbit() & (uint8_t(1) << v0selector::kGamma)) {
      //   fillskimv0table(v0,negTrack,collision,posTrack.tpcNSigmaEl(),posTrack.tofNSigmaEl(),o2::track::PID::Electron);
      // }
      // if (bool(posTrack.pidbit() & (1 << v0selector::kK0S))) {
      //   fillskimv0table(v0,posTrack,collision,posTrack.tpcNSigmaPi(),posTrack.tofNSigmaPi(),o2::track::PID::Pion);
      // }
      // if (bool(negTrack.pidbit() & (1 << v0selector::kK0S))) {
      //   fillskimv0table(v0,negTrack,collision,negTrack.tpcNSigmaPi(),negTrack.tofNSigmaPi(),o2::track::PID::Pion);
      // }
      // if (bool(posTrack.pidbit() & (1 << v0selector::kLambda))) {
      //   fillskimv0table(v0,posTrack,collision,posTrack.tpcNSigmaPr(),posTrack.tofNSigmaPr(),o2::track::PID::Proton);
      // }
      // if (bool(negTrack.pidbit() & (1 << v0selector::kLambda))) {
      //   fillskimv0table(v0,negTrack,collision,negTrack.tpcNSigmaPi(),negTrack.tofNSigmaPi(),o2::track::PID::Pion);
      // }
      // if (bool(posTrack.pidbit() & (1 << v0selector::kAntiLambda))) {
      //   fillskimv0table(v0,posTrack,collision,posTrack.tpcNSigmaPi(),posTrack.tofNSigmaPi(),o2::track::PID::Pion);
      // }
      // if (bool(negTrack.pidbit() & (1 << v0selector::kAntiLambda))) {
      //   fillskimv0table(v0,negTrack,collision,negTrack.tpcNSigmaPr(),negTrack.tofNSigmaPr(),o2::track::PID::Proton);
      // }
    }
  }
  //end
  ...
  void processQA(soa::Join<aod::Collisions, aod::EvSels>::iterator const& collision, soa::Join<FullTracksExt, aod::V0Bits> const& tracks)
  {
    ...
    for (auto& track : tracks) {
      ...
      if (track.pidbit() > 0) {
        ...
        //new add
        registry.fill(HIST("hTrackPidbit"), track.pidbit());
        //new add
      }
  ...
  PROCESS_SWITCH(trackPIDQA, processTree, "get v0 tree form barrel tracks", true);
};
