...
AnalysisCompositeCut* o2::aod::dqcuts::GetCompositeCut(const char* cutName)
{
  ...
  if (!nameStr.compare("PIDCalib_bk")) {
  ...
  if (!nameStr.compare("PIDCalib2")) {
    cut->AddCut(GetAnalysisCut("pidcalib_ele2"));
    return cut;
  }

  if (!nameStr.compare("PIDCalib_test")) {
    cut->AddCut(GetAnalysisCut("v0basic"));
    return cut;
  } 

  if (!nameStr.compare("PIDCalib_testbase")) {
    cut->AddCut(GetAnalysisCut("pidcalib_base"));
    return cut;
  } 
...
AnalysisCut* o2::aod::dqcuts::GetAnalysisCut(const char* cutName)
{
  ...
  if (!nameStr.compare("v0basic_test")) {
    cut->AddCut(VarManager::kPt, 0.0, 1000.0);
    return cut;
  }


  if (!nameStr.compare("v0basic")) {
  ...
  if (!nameStr.compare("pidcalib_proton")) {
    cut->AddCut(VarManager::kIsProtonLeg, 0.5, 1.5, false);
    return cut;
  }

  if (!nameStr.compare("pidcalib_base")) {
    cut->AddCut(VarManager::kIsUsedLeg, 0.5, 1.5, false);
    return cut;
  }

  if (!nameStr.compare("pidcalib_ele2")) {
    cut->AddCut(VarManager::kIsGlobalTrack, 0.5, 1.5, false);
    return cut;
  }
