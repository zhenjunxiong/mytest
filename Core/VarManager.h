...
class VarManager : public TObject
{
 ...
 public:
  ...
  //new add
  template <int pairType, typename T1, typename T2>
  static void FillPairV0(T1 const& t1, T2 const& t2, float* values = nullptr);
  //end
...
void VarManager::FillTrack(T const& track, float* values)
{
  ...
  if constexpr ((fillMap & Track) > 0 || (fillMap & Muon) > 0 || (fillMap & ReducedTrack) > 0 || (fillMap & ReducedMuon) > 0) {
    ...
    if constexpr ((fillMap & ReducedTrack) > 0 && !((fillMap & Pair) > 0)) {
      ...
      //new add
      values[kIsUsedLeg] = bool((values[kIsLegFromGamma] > 0) || (values[kIsLegFromK0S] > 0) || (values[kIsLegFromLambda]) || (values[kIsLegFromAntiLambda]));
      values[kIsElectronLeg] = bool(values[kIsLegFromGamma]);
      values[kIsPionLeg] = bool((values[kIsLegFromK0S] > 0) || (values[kIsLegFromLambda] * track.sign() > 0) || (values[kIsLegFromAntiLambda] * (-track.sign()) > 0));
      values[kIsProtonLeg] = bool((values[kIsLegFromAntiLambda] * track.sign() > 0) || (values[kIsLegFromLambda] * (-track.sign()) > 0));
      //end
    }
  }
...
//new add
template <int pairType, typename T1, typename T2>
void VarManager::FillPairV0(T1 const& t1, T2 const& t2, float* values)
{
  if (!values) {
    values = fgValues;
  }
  float momTot = std::sqrt(std::pow((t1.px() + t2.px()),2) + std::pow((t1.py() + t2.py()),2) + std::pow((t1.pz() + t2.pz()),2));
  float lQlPos = (t1.px()*(t1.px()+t2.px())+t1.py()*(t1.py()+t2.py())+t1.pz()*(t1.pz()+t2.pz()))/momTot;
  float lQlNeg = (t2.px()*(t1.px()+t2.px())+t2.py()*(t1.py()+t2.py())+t2.pz()*(t1.pz()+t2.pz()))/momTot;
  float p2_pos = std::pow(t1.px(),2) + std::pow(t1.py(),2) + std::pow(t1.pz(),2);
  float p2_neg = std::pow(t2.px(),2) + std::pow(t2.py(),2) + std::pow(t2.pz(),2);

  values[kalphav0] = (t1.sign() > 0 ? (lQlPos - lQlNeg) / (lQlPos + lQlNeg) : (lQlNeg - lQlPos) / (lQlPos + lQlNeg)); // alphav0
  values[kqtarmv0] = (t1.sign() > 0 ? (std::sqrt(p2_neg - lQlNeg * lQlNeg)) : (std::sqrt(p2_pos - lQlPos * lQlPos))); // qtarm

  bool isgammapair = bool(t1.filteringFlags() & t2.filteringFlags() & (uint64_t(1) << 2));
  bool isk0spair = bool(t1.filteringFlags() & t2.filteringFlags() & (uint64_t(1) << 3));
  bool islambdapair = bool(t1.filteringFlags() & t2.filteringFlags() & (uint64_t(1) << 4));
  bool isantilambdapair = bool(t1.filteringFlags() & t2.filteringFlags() & (uint64_t(1) << 5));

  //values[kisv0] = bool(t1.pairindex() == t2.pairindex());

  if (isgammapair){
    values[kv0type] = 1;//gamma v0
  }
  else if (isk0spair){
    values[kv0type] = 2;//k0s v0
  }
  else if (islambdapair){
    values[kv0type] = 3;//lambda v0
  }
  else if (isantilambdapair){
    values[kv0type] = 4;//antilambda v0
  }
}
//end
