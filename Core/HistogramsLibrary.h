...
void o2::aod::dqhistograms::DefineHistograms(HistogramManager* hm, const char* histClass, const char* groupName, const char* subGroupName)
{
  ...
  if (groupStr.Contains("track")) {
    ...
    if (subGroupStr.Contains("tpcpid")) {
      ...
      //new add
      hm->AddHistogram(histClass, "TPCnSigPion_pIN", "TPC n-#sigma(pion) vs pIN", false, 200, 0.0, 10.0, VarManager::kPin, 100, -5.0, 10.0, VarManager::kTPCnSigmaPi);
      hm->AddHistogram(histClass, "TPCnSigPr_pIN", "TPC n-#sigma(proton) vs pIN", false, 200, 0.0, 10.0, VarManager::kPin, 100, -5.0, 10.0, VarManager::kTPCnSigmaPr);
      //end
    ...
    if (subGroupStr.Contains("tpcpostcalibelectron")) {
      ...
      int varsPIDnSigEle[kNvarsPID] = {VarManager::kTPCnSigmaEl, VarManager::kPin, VarManager::kEta};                                                          
      ...
      nSigBinLimits[2] = TArrayD(kEtaNbins+1, etaBinLimsI);                                                                                                  

      hm->AddHistogram(histClass, "nSigmaTPCelectron", "TPC n_{#sigma}(e) Vs Pin Vs Eta", kNvarsPID, varsPIDnSigEle, nSigBinLimits);
    }
      if (subGroupStr.Contains("tpcpostcalibpion")) {
      const int kNvarsPID = 3;
      const int kTPCnsigmaNbins = 70;
      double tpcNsigmaBinLims[kTPCnsigmaNbins+1];
      for(int i=0; i<=kTPCnsigmaNbins;++i)
      ...
      int varsPIDnSigEle[kNvarsPID] = {VarManager::kTPCnSigmaPi, VarManager::kPin, VarManager::kEta};                                                          
      TArrayD nSigBinLimits[kNvarsPID];                                                                                                                       
      nSigBinLimits[0] = TArrayD(kTPCnsigmaNbins+1, tpcNsigmaBinLims);
      nSigBinLimits[1] = TArrayD(kPinEleNbins+1, pinEleBinLims);                                                                                             
      nSigBinLimits[2] = TArrayD(kEtaNbins+1, etaBinLimsI);                                                                                                  

      hm->AddHistogram(histClass, "nSigmaTPCpion", "TPC n_{#sigma}(pion) Vs Pin Vs Eta", kNvarsPID, varsPIDnSigEle, nSigBinLimits);
    }
      if (subGroupStr.Contains("tpcpostcalibproton")) {
      const int kNvarsPID = 3;
      const int kTPCnsigmaNbins = 70;
      double tpcNsigmaBinLims[kTPCnsigmaNbins+1];
      for(int i=0; i<=kTPCnsigmaNbins;++i)
      ...
      int varsPIDnSigEle[kNvarsPID] = {VarManager::kTPCnSigmaPr, VarManager::kPin, VarManager::kEta};                                                          
      TArrayD nSigBinLimits[kNvarsPID];                                                                                                                       
      nSigBinLimits[0] = TArrayD(kTPCnsigmaNbins+1, tpcNsigmaBinLims);
      nSigBinLimits[1] = TArrayD(kPinEleNbins+1, pinEleBinLims);                                                                                             
      nSigBinLimits[2] = TArrayD(kEtaNbins+1, etaBinLimsI);                                                                                                  

      hm->AddHistogram(histClass, "nSigmaTPCproton", "TPC n_{#sigma}(proton) Vs Pin Vs Eta", kNvarsPID, varsPIDnSigEle, nSigBinLimits);
    }
